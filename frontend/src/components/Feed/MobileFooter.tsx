...
{/* <NavLink to="/" aria-label="Home">
  {({ isActive }) => (
    <div className={`relative ${isActive ? 'after:absolute after:inset-0 after:rounded after:bg-black/10' : ''}`}>
      <Home className="h-6 w-6" />
    </div>
  )}
</NavLink> */}
...
{/* <NavLink to="/search" aria-label="Search">
  {({ isActive }) => (
    <div className={isActive ? 'bg-primary/20 p-1 rounded-full' : 'p-1'}>
      <Search className="h-6 w-6" />
    </div>
  )}
</NavLink> */}
<NavLink to="/search" aria-label="Search">
  {({ isActive }) => (
    ...
    //  isActive? <BsSearchHeart className={` md:h-6 md:w-6 h-5 w-5 transition-colors duration-200`} />
    //  : <BsSearch className="md:h-6 md:w-6 h-5 w-5 " />
  )}
</NavLink>
